    ...
    void render(GLFWwindow* window, unsigned int shaderProgram, float deltaTime, float lastFrame) {
     ...
     Enemies.emplace_back(Enemy0);
        ...
        while (!glfwWindowShouldClose(window))
            ...
            {
                glm::vec3 Enemy0direction = glm::normalize(componentManager.getComponent<transform_component>(Player.ID).PlayerPos - componentManager.getComponent<transform_component>(enemy.ID).PlayerPos);
                                    componentManager.getComponent<movement_component>(enemy.ID).Velocity = Enemy0direction * 0.5f;
            }
            

            for (auto enemy : Enemies)
            {
                if(coll.SphereSphereCollision(componentManager.getComponent<transform_component>(Player.ID).PlayerPos ,componentManager.getComponent<transform_component>(enemy.ID).PlayerPos) == true)
                            {
                    componentManager.getComponent<movement_component>(Player.ID).Velocity *= glm::vec3(-1);
                    componentManager.getComponent<health_component>(Player.ID).health -= 1;

                    componentManager.getComponent<health_component>(enemy.ID).health --;
                    if (componentManager.getComponent<health_component>(enemy.ID).health <= 0)
                    {
                        std::cout << "Enemy is dead" << std::endl;
                        entityManager.destroy_entity(enemy);
                        componentManager.getComponent<transform_component>(enemy.ID).PlayerPos = glm::vec3(100);
                    }
